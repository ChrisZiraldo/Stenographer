...
export function listMeetings({ limit = 50, offset = 0, tag = null } = {}) {
  const db = getDb();
  let query = `
    SELECT m.*,
           n.human_doc_text,
...
export function upsertTodo(todo) {
  ...
  const now = Date.now();
  db.prepare(`
    INSERT INTO todos (id, meeting_id, text, done, owner, due, source, position, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
...
function updateFts(meetingId) {
  ...
  const notes = db.prepare('SELECT human_doc_text, enhanced_md FROM notes WHERE meeting_id = ?').get(meetingId);
  const segments = db.prepare(
    'SELECT text FROM transcript_segments WHERE meeting_id = ? ORDER BY start_ms ASC'
  ).all(meetingId);

  const transcriptText = segments.map((s) => s.text).join(' ');
  const notesText = [notes?.human_doc_text, notes?.enhanced_md].filter(Boolean).join(' ');
